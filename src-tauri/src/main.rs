...
/// Detect the actual Ubuntu distribution name installed in WSL.
/// Handles versioned names like "Ubuntu-24.04", "Ubuntu-22.04", etc.
/// Falls back to "Ubuntu" if no match is found.
#[cfg(target_os = "windows")]
fn detect_wsl_ubuntu_distro() -> String {
    ...
    if let Ok(o) = output {
        ...
        let raw = &o.stdout;
        let stdout_str = if raw.len() >= 2 && raw[0] == 0xFF && raw[1] == 0xFE {
            // UTF-16LE with BOM
            ...
            // Try UTF-16LE without BOM (common on newer Windows)
            let u16s: Vec<u16> = raw
                .chunks_exact(2)
                .map(|c| u16::from_le_bytes([c[0], c[1]]))
                .collect();
            let decoded = String::from_utf16_lossy(&u16s);
            if decoded.contains("Ubuntu") || decoded.contains("ubuntu") {
                decoded
            } else {
        ...
        // Find the first line that starts with "Ubuntu" (case-insensitive prefix match)
        for line in stdout_str.lines() {
            let trimmed = line.trim().trim_matches('\0');
            if trimmed.to_lowercase().starts_with("ubuntu") && !trimmed.is_empty() {
                return trimmed.to_string();
            }
        }
    }

    // Fallback to "Ubuntu" if detection fails
    "Ubuntu".to_string()
}
...
fn wait_for_wsl_ready(timeout_secs: u64) -> Result<(), String> {
    let distro = detect_wsl_ubuntu_distro();
    let start = std::time::Instant::now();
    let timeout = Duration::from_secs(timeout_secs);
    while start.elapsed() < timeout {
        let output = Command::new("wsl")
            .args(["-d", &distro, "-u", "root", "--", "echo", "ready"])
            .output();
        if let Ok(o) = output {
            if o.status.success() {
    ...
    Err(format!("WSL {} not ready after {} seconds", distro, timeout_secs))
}
...
fn ensure_wsl2_installed() -> Result<(), String> {
    ...
    let user_setup = Command::new("wsl")
        .args(["-d", &distro, "-u", "root", "--", "/bin/bash", "-c",
    ...
    // Terminate distro so wsl.conf takes effect on next launch
    ...
    // Wait for distro to come back with the new default user
    wait_for_wsl_ready(30).map_err(|e| {
        format!("WSL {} failed to restart after user configuration: {}", distro, e)
    })?;
...
fn wsl_root_command(cmd: &str) -> Result<String, String> {
    let distro = detect_wsl_ubuntu_distro();
    let output = Command::new("wsl")
        .args(["-d", &distro, "--user", "root", "--", "/bin/bash", "-c", cmd])
        .output()
...
fn shell_command(cmd: &str) -> Result<String, String> {
    ...
    let distro = detect_wsl_ubuntu_distro();
    #[cfg(target_os = "windows")]
    let (shell, args) = ("wsl", vec!["-d", &distro, "--", "/bin/bash", "-c"]);
...
mod tests {
    ...
    fn test_wsl_root_command_uses_explicit_distro() {
        // wsl_root_command should use `-d <distro>` for robustness
        // Verify the expected argument structure (distro name is dynamic)
        let cmd = "echo hello";
        let distro = "Ubuntu-24.04"; // example versioned distro
        let expected_args = vec!["-d", distro, "--user", "root", "--", "/bin/bash", "-c", cmd];
        assert_eq!(expected_args[0], "-d");
        assert!(expected_args[1].starts_with("Ubuntu"));
    ...
    #[test]
    fn test_detect_wsl_ubuntu_distro_parsing() {
        // Simulate the parsing logic from detect_wsl_ubuntu_distro
        // to verify it correctly picks versioned distro names
        let test_cases = vec![
            ("Ubuntu-24.04\nDocker\ndebian\n", "Ubuntu-24.04"),
            ("Ubuntu-22.04\nUbuntu\n", "Ubuntu-22.04"),
            ("Ubuntu\n", "Ubuntu"),
        ...
        for (input, expected) in test_cases {
            let mut found = "Ubuntu".to_string();
    ...
    fn test_wait_for_wsl_ready_command_args() {
        ...
        let distro = "Ubuntu-24.04"; // example versioned distro
        let expected_args = ["-d", distro, "-u", "root", "--", "echo", "ready"];
        assert_eq!(expected_args[0], "-d");
        assert!(expected_args[1].starts_with("Ubuntu"));
